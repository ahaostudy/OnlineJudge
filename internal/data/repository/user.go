...
func GetUser(id int64) (*model.User, error) {
	user := new(model.User)
	err := data.DB.Where("id = ?", id).First(user).Error
	return user, err
}
...
func GetUserByEmail(email string) (*model.User, error) {
	user := new(model.User)
	err := data.DB.Where("email = ?", email).First(user).Error
	return user, err
}
...
func UpdateUser(id int64, user map[string]any) error {
	return data.DB.Model(new(model.User)).Where("id = ?", id).Updates(user).Error
}
